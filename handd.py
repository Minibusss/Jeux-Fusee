...
def main():
    pTime = 0
    cTime = 0
    cap = cv2.VideoCapture(0)  # Utilisez 0 pour la caméra par défaut
    if not cap.isOpened():
    ...
    try:
        ...
        print(f"Erreur lors de l'initialisation de handDetector : {e}")
        return
    ...
    while True:
        ...
        if cv2.waitKey(1) == 27:  # Échapper pour quitter
            break
    cap.release()
